...
export default function Payment({ table, getTables }) {
  ...
  function paymant() {
    if (
      Number(pay) + Number(table.pay) + Number(table.discount) >=
      Number(table.totalPrice)
    ) {
      Axios.post("/tables/pay", {
        id: table._id,
        pay: Number(table.pay) + (Number(pay) - Number(change)),
      }).catch((err) => console.log(err));
      Axios.post("/tables/close", { id: table._id, close: 1 }).catch((err) =>
        console.log(err)
      );
      ...
      if (pay > 0) {
        Axios.post("/tables/pay", {
          id: table._id,
          pay: Number(table.pay) + Number(pay),
        })
          .then(() => getTables())
          .catch((err) => console.log(err));
      }
        ...
        <div className="col-6">
          <div className="card">
            ...
            <div className="card-body">
              <label className="form-label" htmlFor="DiscountPros">
                הנחה ב%
              </label>
        ...
        <div className="col-6">
          <div className="card">
            ...
            <div className="card-body">
              ...
              <strong>הנחה:</strong>
              <span className="float-end">
                {currencyFormat.format(table.discount)}
              </span>
              <br />
              <strong>שולם:</strong>
              <span className="float-end">
                {currencyFormat.format(table.pay)}
              </span>
              <br />
              <strong>סכום לתשלום:</strong>
              <span className="float-end">
                {currencyFormat.format(
                  table.totalPrice - table.pay - table.discount
                )}
              </span>
              <br />
            </div>
        ...
        <div className="col-12">
          <div className="card">
            ...
            <div className="card-body">
              ...
              />
              <label className="form-label" htmlFor="cahgne">
                עודף
              </label>
              <input
                type="number"
