...
export default function Menu({tid, prodects, getOrders, totalBill}) {
    ...
    function searchHandel(key) {
      ...
      categorys.forEach(cate => {
        document.querySelector(`#category-tab-${cate._id}`).classList.remove("active")
        document.querySelector(`#category-tab-${cate._id}`).classList.remove("show")
      });
    }

    function hideSearch(params) {
      document.querySelector("#category-tab-search").classList.remove("active", "show");
    ...
    <div className="card-body">
      ...
      <div className="row">
        ...
        <div
            ...
            >
            <strong>תוצאות חיפוש:</strong><br />
            {!search ? null : (
              prodects.filter(
              (prodect) => prodect.title.indexOf(search) > -1)
              .map((prodect) => (
                <button
                type="button"
                className="btn btn-lg btn-outline-primary me-1 mb-1"
                key={prodect._id}
                onClick={(e) => addOrder(prodect._id)}
              >
                {prodect.title}
          ...
          {categorys.map((category) => (
            ...
            >
              <div className="d-grid gap-2 d-md-block">
                {prodects
                  .filter(
                    (prodect) => prodect.cid === category._id
                  )
                  .map((prodect) => (
                    <button
                      type="button"
