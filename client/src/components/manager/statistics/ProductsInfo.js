...
export default function productsInfo({ orders, products }) {
  ...
  function Sold(pid) {
    let numberSold;
    if (pid) {
      numberSold = orders.filter(
        (ord) =>
          ord.pid === pid &&
          Date.parse(ord.createdAt) >= startDate &&
          Date.parse(ord.createdAt) <= endDate
      ).length;
    }
    return numberSold;
  }
          ...
          <table className="table table-striped">
            <thead>
              <tr className="text-center">
                ...
                <th scope="col">כמות שנמכרו</th>
                <th scope="col">שווי מוערך</th>
              </tr>
            </thead>
            <tbody>
              {products.map((product) => (
                <tr className="text-center" key={product._id}>
                  ...
                  <td>{Sold(product._id)}</td>
                  <td>
                    {currencyFormat.format(product.price * Sold(product._id))}
                  </td>
                </tr>
              ))}
            </tbody>
