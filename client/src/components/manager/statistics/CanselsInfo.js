...
export default function CanselsInfo({orders, prodects, tables}) {
    const [affterSort, setAffterSort] = useState([]);
    const [startDate, setStartDate] = useState(0);
    const [endDate, setEndDate] = useState(Date.now()+30000);

    useEffect(() => {
        const affter = orders.sort((a,b) => (a.createdAt < b.createdAt) ? 1 : ((b.createdAt < a.createdAt) ? -1 : 0));
    ...
    return (
        ...
        <table className="table table-striped">
            ...
            <tbody>
                {affterSort.filter(ord => ord.cansel === 1).map(order => (
                <tr className="text-center" key={order._id}>
                    <td>{prodectInfo("title", order.pid)}</td>
                    ...
                    <td>{currencyFormat.format(prodectInfo("price", order.pid))}</td>
                </tr>
                ))}
            </tbody>
        </table>
