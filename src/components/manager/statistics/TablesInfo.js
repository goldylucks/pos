...
export default function TablesInfo({tables}) {
    ...
    useEffect(() => {
        const affter = tables.sort((a,b) => (a[sortBy] < b[sortBy]) ? 1 : ((b[sortBy] < a[sortBy]) ? -1 : 0));
    ...
    useEffect(() => {
        const affter = tables
        .filter(el => Date.parse(el.createdAt) >= startDate && Date.parse(el.createdAt) <= endDate)
        ...
        <table className="table table-striped">
            ...
            <tr className="text-center">
                <th scope="col">
                    מספר שולחן
                    <i 
                        className={`fas fa-sort ms-1 aling-middle ${sortBy === "number" ? "text-primary" : null}`}
                ...
                <th scope="col">
                    נפתח בתאריך
                    <i 
                        className={`fas fa-sort ms-1 aling-middle ${sortBy === "createdAt" ? "text-primary" : null}`}
                ...
                <th scope="col">
                    נסגר בתאריך
                    <i 
                        className={`fas fa-sort ms-1 aling-middle ${sortBy === "updatedAt" ? "text-primary" : null}`}
                ...
                <th scope="col">
                    חשבון
                    <i 
                        className={`fas fa-sort ms-1 aling-middle ${sortBy === "totalPrice" ? "text-primary" : null}`}
                        onClick={() => setSortBy("totalPrice")}                    
                    ></i>                </th>
                <th scope="col">
                    הנחה
                    <i 
                        className={`fas fa-sort ms-1 aling-middle ${sortBy === "discount" ? "text-primary" : null}`}
                ...
                <th scope="col">
                    שולם
                    <i
                        className={`fas fa-sort ms-1 aling-middle ${sortBy === "pay" ? "text-primary" : null}`}
