...
export default function DiscountInfo({tables}) {
    ...
    useEffect(() => {
        const affter = tables.sort((a,b) => (a.createdAt < b.createdAt) ? 1 : ((b.createdAt < a.createdAt) ? -1 : 0));
    ...
    return (
        ...
        <table className="table table-striped">
            ...
            <tbody>
                {affterSort.filter(tab => tab.discount > 0).map(table => (
                <tr className="text-center" key={table._id}>
                    ...
                    <td>{Math.round((table.discount / table.totalPrice) * 100)} %</td>
                </tr>
                ))}
            </tbody>
        </table>
